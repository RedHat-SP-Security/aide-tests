...
function checkUpdateAide {
    rlRun "aideAsUser $testUser --update -c $TEST_DIR/aide.conf" $1 "Updating AIDE database as $testUser"
...
rlJournalStart
    rlPhaseStartSetup
        ...
        rlRun "aideAsUser $testUser -i -c $TEST_DIR/aide.conf" 0 "Initializing AIDE database as $testUser"
        rlRun "mv -f $TEST_DIR/db/aide.db.new.gz $TEST_DIR/db/aide.db.gz" 0 "Moving database with new data"
    rlPhaseEnd
    ...
    rlPhaseStartTest "Making changes and restoring them"
        ...
        rlRun "aideAsUser $testUser --check -c $TEST_DIR/aide.conf" 4 "Checking changes"
        rlRun "echo 'Random text' > $TEST_DIR/data/random.txt"
